...
Customer.create = (newCustomer, result) => {
	sql.query("INSERT INTO tips SET ?", newCustomer, (err, res) => {
		if (err) {
			console.log("error: ", err);
			result(err, null);
			return;
...
Customer.findById = (customerId, result) => {
	sql.query(`SELECT * FROM tips WHERE id = ${customerId}`, (err, res) => {
		if (err) {
			console.log("error: ", err);
			result(err, null);
			return;
...
Customer.updateById = (id, customer, result) => {
	sql.query(
		"UPDATE tips SET level_col = ?, cvss = ?, title = ?, vulnerability = ?, solution = ?, reference_col = ? WHERE id = ?",
		[
			customer.level_col,
			customer.cvss,
			customer.title,
...
Customer.remove = (id, result) => {
	sql.query("DELETE FROM tips WHERE id = ?", id, (err, res) => {
		if (err) {
			console.log("error: ", err);
			result(null, err);
			return;
...
Customer.removeAll = (result) => {
	sql.query("DELETE FROM tips", (err, res) => {
		if (err) {
			console.log("error: ", err);
			result(null, err);
			return;
